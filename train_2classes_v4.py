...
# -*- coding: utf-8 -*-
"""
- Tversky参数激进调整 (FP权重: 0.6→0.4, FN权重: 0.4→0.6) 
- 采样策略平衡 (病灶过采样: 2.5→1.5, 背景欠采样: 0.3→0.5)
- 强化边界收缩模块 (权重: 0.05→0.12)
"""
...
class DSCEnhancedConfig:
    ...
    USE_BOUNDARY_AWARE_DICE = False      
    USE_TVERSKY_LOSS = True             # 保留Tversky Loss来惩罚FP
    
    CE_WEIGHT = 0.0                     # 交叉熵损失 
    FOCAL_WEIGHT = 0.5                  # Focal损失
    DICE_WEIGHT = 0.0                  # 标准Dice损失
    BOUNDARY_AWARE_DICE_WEIGHT = 0.0   # 边界感知Dice损失
    TVERSKY_WEIGHT = 0.6                # Tversky损失
    BOUNDARY_LOSS_WEIGHT = 0.0         # 边界距离损失
    HAUSDORFF_LOSS_WEIGHT = 0.2         # Hausdorff距离损失 
    ...
    # 激进平衡采样策略
    LESION_OVERSAMPLE_FACTOR = 1.5      
    BACKGROUND_UNDERSAMPLE_FACTOR = 0.5  
